...
import { useGetJob, useDeleteJob, getGetJobStatsQueryKey, getListJobsQueryKey } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { ArrowLeft, Trash2, ExternalLink, CheckCircle2, Loader2, Circle, XCircle, Play } from "lucide-react";
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <Button
            ...
            className="text-destructive hover:text-destructive hover:border-destructive/50 font-mono text-xs shrink-0"
        ...
        <div className="divide-y divide-border">
          {(job.steps ?? []).map((step, idx) => {
            ...
            return (
              <div
                key={step.id}
                className={`flex items-start gap-4 p-5 transition-colors ${step.status === "running" ? "bg-blue-500/5" : ""}`}
              ...
              >
                ...
                <div className="flex flex-col items-center gap-0 shrink-0 pt-0.5">
                  ...
                  {!isLast && (
                    <div className={`w-px flex-1 mt-2 min-h-[24px] ${step.status === "complete" ? "bg-primary/40" : "bg-border"}`} />
                  )}
                </div>
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    {step.status !== "pending" && (
                      <span className={`text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                        ...
                        step.status === "running" ? "text-blue-400 bg-blue-400/10 border-blue-400/20" :
